...
class RecordObject {
    ...
    public function &findAll($where = false, $params = false, $more_sql = false) {
        ...
        //-----------------pick up cache strategy[start]------------------
        $result = $this->cacheStrategy($sql,$params);
        if(!empty($result))
        {
        	if($result == 'empty')//can not find in memory and return null to system
        	{
        		return null;
        	}
        	if($result != 'notmatch')//fetch memory data,retrun value is 'notmatch' that means fetching data from database
        	{
        		return $result;
        	}
        }
        //-----------------pick up cache strategy[end]-------------------
    ...
    protected function _insert($get_insert_id = '') {
        ...
        try {
            ...
            throw new RecordException('Failed executing(insert) update!'."\n"
                                      .$ex->getMessage());
        }
    }
    ...
    /**
     * The definition of cache strategy
     * @param $param1
     * @param $param2
     */
