...
//          return @copy($remoteurl,  $localcsvfile);
	...
	public static function ResetTplData(){
		...
		foreach($rows as $row){
			...
			$val = $_row_val[0];
			if($val != Config::$tbl_prefix."users" &&$val != Config::$tbl_prefix."user_extends" && $val != Config::$tbl_prefix."parameters"&& $val != Config::$tbl_prefix."admin_menu_categories"&& $val != Config::$tbl_prefix."admin_menu_items"&& $val != Config::$tbl_prefix."languages" && $val != Config::$tbl_prefix."payment_accounts" && $val != Config::$tbl_prefix."payment_providers" && $val != Config::$tbl_prefix."roles"){
				$sql = "delete from ".$val;
				$rs =& $db->query($sql);
			}
		}
